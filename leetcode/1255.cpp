...
int solve(const vector<string>& words, vector<int>& scores, vector<string>& subset, size_t words_i = 0, int ans = 0)
{
    {
        int score = 0;
    ...
    for (size_t j = words_i; j < words.size(); j++) {
        ...
        ans = max(ans, solve(words, scores, subset, j + 1));
        subset.pop_back();
...
int solve(const vector<string>& words, const vector<int>& scores, vector<string>& subset, size_t words_i = 0, int ans = 0)
{
    {
        int score = 0;
        for (const string& word : subset) for (const char letter : word) score += scores[letter - 'a'];
    ...
    ans = max(ans, solve(words, scores, subset, words_i + 1, ans));
