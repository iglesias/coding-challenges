...
struct p {
  ...
  // there'll be points created whose r and/or c are out of bounds.
  int r, c;
...
struct number {
  ...
  int is_part; // -1 -> uninitialized, 0 -> no, 1 -> yes. FIXME std::optional
               // for uninitialized.
...
std::vector<::number> extract_numbers(input_t const &input) {
  ...
  while (i < input.at(0).length()) {
    ::p start;
    bool busy{false}; // parsing state, is a number being processed?
    unsigned j{0};
    while (j < input.size()) {
...
int part_numbers_sum(const input_t &input, std::vector<::number> &numbers) {
  int ans{0};

  for (auto &number : numbers) {
    const int start_r{number.start.r - 1}, start_c{number.start.c - 1};
    const int end_r{number.start.r + 1}, end_c{number.end.c + 1};
    number.is_part = 0;
    ...
    for (int i{start_r}; i <= end_r && !number.is_part; i++) {
      ...
      for (int j{start_c}; j <= end_c && !number.is_part; j++) {
        if (j < 0)
          continue;
        if (j > (int)input[0].length() - 1)
          break;

        if (input[i][j] != '.' and
            not('0' <= input[i][j] and input[i][j] <= '9'))
          number.is_part = 1;
      }
    }
...
int part_two(const input_t &input, const std::vector<::number> &numbers) {
  int ans{0};
  for (int i{0}; i < (int)input.size() - 1; i++) for (int j{0}; j < (int)input[0].length(); j++)
      if (input[i][j] == '*') {
        ...
        for (const auto &number : numbers) {
          if (!number.is_part)
            continue;
          if (i < number.start.r - 1 || i > number.start.r + 1)
            continue;
          if (j < number.start.c - 1 || j > number.end.c + 1)
            continue;
          adjacent_part_number_cnt++;
          if (adjacent_part_number_cnt > 2)
            break;
